...
static void hiddenConsoleW(wchar_t const *aString,
								   ...
								   yesnocancel */
{
	...
	if (quoteDetectedW(aTitle))
		return tinyfd_messageBoxW(L"INVALID TITLE WITH QUOTES", aMessage,
								  aDialogType, aIconType, aDefaultButton);
	if (quoteDetectedW(aMessage))
		return tinyfd_messageBoxW(aTitle, L"INVALID MESSAGE WITH QUOTES",
								  aDialogType, aIconType, aDefaultButton);
	...
	wchar_t const *aDefaultInput) /* L"" , if NULL it's a passwordBox */
{
	...
	if (quoteDetectedW(aDefaultInput))
		return tinyfd_inputBoxW(aTitle, aMessage,
								L"INVALID DEFAULT_INPUT WITH QUOTES");
	...
	wchar_t const *aSingleFilterDescription) /* NULL or "image files" */
{
	...
	if (quoteDetectedW(aTitle))
		return tinyfd_saveFileDialogW(
			L"INVALID TITLE WITH QUOTES", aDefaultPathAndFile,
			aNumOfFilterPatterns, aFilterPatterns, aSingleFilterDescription);
	if (quoteDetectedW(aDefaultPathAndFile))
		return tinyfd_saveFileDialogW(
			aTitle, L"INVALID DEFAULT_PATH WITH QUOTES", aNumOfFilterPatterns,
			aFilterPatterns, aSingleFilterDescription);
	if (quoteDetectedW(aSingleFilterDescription))
		return tinyfd_saveFileDialogW(
			aTitle, aDefaultPathAndFile, aNumOfFilterPatterns, aFilterPatterns,
			L"INVALID FILTER_DESCRIPTION WITH QUOTES");
	for (i = 0; i < aNumOfFilterPatterns; i++) {
		if (quoteDetectedW(aFilterPatterns[i]))
			return tinyfd_saveFileDialogW(L"INVALID FILTER_PATTERN WITH QUOTES",
										  aDefaultPathAndFile, 0, NULL, NULL);
	}
								  ...
								  return */
{
	...
	if (quoteDetectedW(aTitle))
		return tinyfd_openFileDialogW(L"INVALID TITLE WITH QUOTES",
									  aDefaultPathAndFile, aNumOfFilterPatterns,
									  aFilterPatterns, aSingleFilterDescription,
	...
	if (quoteDetectedW(aSingleFilterDescription))
		return tinyfd_openFileDialogW(
			aTitle, aDefaultPathAndFile, aNumOfFilterPatterns, aFilterPatterns,
			L"INVALID FILTER_DESCRIPTION WITH QUOTES", aAllowMultipleSelects);
	for (i = 0; i < aNumOfFilterPatterns; i++) {
		if (quoteDetectedW(aFilterPatterns[i]))
			return tinyfd_openFileDialogW(L"INVALID FILTER_PATTERN WITH QUOTES",
										  aDefaultPathAndFile, 0, NULL, NULL,
										  aAllowMultipleSelects);
	}
...
static int __stdcall BrowseCallbackProcW(HWND hwnd,
										 UINT uMsg,
										 LPARAM lp,
										 LPARAM pData) {
	switch (uMsg) {
	...
	wchar_t const *aDefaultPath) /* NULL or "" */
{
	...
	if (quoteDetectedW(aTitle))
		return tinyfd_selectFolderDialogW(L"INVALID TITLE WITH QUOTES",
										  aDefaultPath);
	if (quoteDetectedW(aDefaultPath))
		return tinyfd_selectFolderDialogW(aTitle,
	...
	unsigned char aoResultRGB[3])		/* { 0 , 0 , 0 } */
{
	...
	if (quoteDetectedW(aTitle))
		return tinyfd_colorChooserW(L"INVALID TITLE WITH QUOTES",
									aDefaultHexRGB, aDefaultRGB, aoResultRGB);
	if (quoteDetectedW(aDefaultHexRGB))
		return tinyfd_colorChooserW(aTitle,
									L"INVALID DEFAULT_HEX_RGB WITH QUOTES",
	...
	char const *aSingleFilterDescription) /* NULL or "image files" */
{
	...
	if (tfd_quoteDetected(aSingleFilterDescription))
		return tinyfd_saveFileDialog(aTitle, aDefaultPathAndFile,
									 aNumOfFilterPatterns, aFilterPatterns,
	...
	if ((!tinyfd_forceConsole || !(GetConsoleWindow() || dialogPresent())) &&
		...
		p = saveFileDialogWinGui(
			lBuff, aTitle, aDefaultPathAndFile, aNumOfFilterPatterns,
	...
	unsigned char aoResultRGB[3])		/* { 0 , 0 , 0 } */
{
	...
	if (tfd_quoteDetected(aDefaultHexRGB))
		return tinyfd_colorChooser(aTitle,
								   "INVALID DEFAULT_HEX_RGB WITH QUOTES",
								   aDefaultRGB, aoResultRGB);
		...
		if (tfd_isDarwin()) {
			if (*strcpy(lTerminalName, "/opt/X11/bin/xterm") &&
				...
				strcat(lTerminalName,
					   " -fa 'DejaVu Sans Mono' -fs 10 -title tinyfiledialogs "
					   "-e ");
...
void tinyfd_beep(void) {
	...
	if (osascriptPresent()) {
		...
		/*strcpy( lDialogString , "pactl load-module module-sine
		 * frequency=440;sleep .3;pactl unload-module module-sine" ) ;*/
		strcpy(lDialogString,
		 ...
		 * --test sine > /dev/tty" ) ;*/
		strcpy(lDialogString,
			   "( speaker-test -t sine -f 440 > /dev/tty )& pid=$!;sleep .4; "
								...
								yesnocancel */
{
 ...
 -e 'else' -e 'return 0' -e 'end if' ");
		...
		if (!strcmp("yesnocancel", aDialogType)) {
			strcat(lDialogString,
				   "; x=$? ;if [ $x = 0 ] ;then echo 1;elif [ $x = 1 ] ;then "
				   "echo 2;else echo 0;fi");
		} else {
...
tinyfdRes=$(cat /tmp/tinyfd.txt);echo $tinyfdBool$tinyfdRes");
			...
			if (aDialogType && !strcmp("yesnocancel", aDialogType)) {
				...
				if (lWasXterm) {
					strcat(lDialogString,
						   "then\n\techo 1\nelse\n\techo 0\nfi "
						   ">/tmp/tinyfd.txt';cat /tmp/tinyfd.txt;rm "
						   "/tmp/tinyfd.txt");
				} else {
					strcat(lDialogString,
...
frontmost of process \\\"Python\\\" to true' ''');");
		...
		if (!xdialogPresent() && !gdialogPresent()) {
			...
			if (!aDefaultInput && !lWasGdialog) {
				strcat(lDialogString,
					   " (sometimes nothing, no blink nor star, is shown in "
					   "text field)");
			}
			strcat(lDialogString, "\" ");
		}
	...
	char const *aSingleFilterDescription) /* NULL or "image files" */
{
 ...
 frontmost of process \\\"Python\\\" to true' ''');");
		...
		if (!xdialogPresent() && !gdialogPresent()) {
			strcat(lDialogString, "--backtitle \"");
			strcat(lDialogString,
				   "tab: focus | /: populate | spacebar: fill text field | ok: "
				   "TEXT FIELD ONLY");
			strcat(lDialogString, "\" ");
		}
		...
		if (lWasGraphicDialog) {
			...
			if (lWasXterm) {
				strcat(lDialogString,
					   "2>/tmp/tinyfd.txt';cat /tmp/tinyfd.txt;rm "
					   "/tmp/tinyfd.txt");
			} else {
	...
	int aAllowMultipleSelects)			  /* 0 or 1 */
{
	...
	if (osascriptPresent()) {
		...
		if (aAllowMultipleSelects) {
			...
			strcat(lDialogString,
				   "-e 'set mystring to mystring & POSIX path of item i of "
				   "mylist' ");
...
to set mycolor to choose color default color {");
		...
		strcat(lDialogString, "}' ");
		strcat(lDialogString,
			   "-e 'set mystring to ((item 1 of mycolor) div 256 as integer) "
			   "as string' ");
		strcat(lDialogString,
			   "-e 'repeat with i from 2 to the count of mycolor' ");
		strcat(lDialogString,
			   "-e 'set mystring to mystring & \" \" & ((item i of mycolor) "
			   "div 256 as integer) as string' ");
		strcat(lDialogString, "-e 'end repeat' ");
		strcat(lDialogString, "-e 'mystring' ");
		strcat(lDialogString, "-e 'on error number -128' ");
		strcat(lDialogString, "-e 'end try'");
