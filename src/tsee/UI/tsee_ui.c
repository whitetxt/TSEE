...
bool TSEE_UI_Init(TSEE *tsee) {
	if (!tsee->init->text) {
		TSEE_Error(
			"Attempted to init TSEE UI without initialising TSEE Text.\n");
		return false;
	}
	if (SDL_SetRenderDrawBlendMode(tsee->window->renderer,
...
bool TSEE_Toolbar_AddButton(TSEE *tsee, char *font, char *text) {
	...
	for (int i = 0; i < idx; i++) {
		sizeBefore +=
			((TSEE_Toolbar_Object *)TSEE_Array_Get(tsee->ui->toolbar, i))
				->text->texture->rect.w +
			16;
	}
 ...
 * @return success status
 */
bool TSEE_Toolbar_AddChild(TSEE *tsee,
						   char *parentName,
						   char *font,
						   char *text,
						   void (*cb)(void *tsee)) {
	...
	child->text->texture->rect = (SDL_Rect){
		parent->text->texture->rect.x + parent->text->texture->rect.w / 2 -
			child->text->texture->rect.w / 2,
